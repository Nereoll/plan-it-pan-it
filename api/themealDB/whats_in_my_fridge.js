...
document.addEventListener('DOMContentLoaded', () => {
  ...
  async function getMealFromIngredients(ingredients) {
    try {
      let meals = new Map(); // Utilisation d'une Map pour éviter les doublons

      for (const ingredient of ingredients) {
        const response = await fetch(`https://www.themealdb.com/api/json/v1/1/filter.php?i=${ingredient}`);
        const data = await response.json();
        if (data.meals) {
          data.meals.forEach(meal => meals.set(meal.idMeal, meal)); // Stocker les repas uniques
        }
      }

      return Array.from(meals.values()); // Retourner tous les repas sans doublons
    } catch (error) {
